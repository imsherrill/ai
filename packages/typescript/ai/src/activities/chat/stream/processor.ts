...
export class StreamProcessor {
  ...
  private handleStepStartedEvent(
    ...
    const stepId = chunk.stepId ?? chunk.stepName
    const activeId = this.getActiveAssistantMessageId()
    if (activeId) {
      const state = this.getMessageState(activeId)
      if (state) {
   ...
   *
   * Accumulates reasoning deltas by stepId and updates the ThinkingPart
   * in the UIMessage. If no STEP_STARTED arrived first, the reasoning messageId
   * is used as a stable fallback stepId.
   */
  private handleReasoningMessageContentEvent(
    ...
    const delta = chunk.delta || ''
    const thinking = (state.thinkingSteps.get(stepId) ?? '') + delta
    state.thinkingSteps.set(stepId, thinking)
    ...
    this.events.onThinkingUpdate?.(messageId, stepId, thinking)
  }
